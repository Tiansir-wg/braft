...
namespace braft
{
    ...
    {
        PeerStatus()
            : valid(false), installing_snapshot(false), next_index(0), last_rpc_send_timestamp(0), flying_append_entries_size(0), readonly_index(0), consecutive_error_times(0)
        {
        }
    ...
    struct NodeStatus
    {
        ...
        bool readonly;
        int64_t term;
        int64_t committed_index;     // 已提交日志索引
        int64_t known_applied_index; // 已apply的日志索引
    ...
    struct NodeOptions
    {
        ...
        // Default: 1000 (1s)
        // 发起选举的超时时间, follower在此时间内未收到leader的消息则从follower状态转化到
        // candidate状态
        ...
        //
        // Default: 1000
        // 如果新节点的last_log_index与leader的last_log_index差值小于catchup_margin则认为该节点已经追上了leader
        int catchup_margin;
