...
namespace braft
{
    ...
    ThroughputSnapshotThrottle::ThroughputSnapshotThrottle(
        ...
        : _throttle_throughput_bytes(throttle_throughput_bytes), _check_cycle(check_cycle), _snapshot_task_num(0), _last_throughput_check_time_us(
    ...
    // 判断是否能够添加一个新的snapshot任务，true表示可以
    bool ThroughputSnapshotThrottle::add_one_more_task(bool is_leader)
    {
        ...
        // 当前正在进行的snapshot任务数已经达到了最大能执行的任务数
