...
namespace braft
{
    ...
    int LocalSnapshotMetaTable::save_to_file(FileSystemAdaptor *fs, const std::string &path) const
    {
        ...
        for (Map::const_iterator
                 iter = _file_map.begin();
             iter != _file_map.end(); ++iter)
        {
    ...
    LocalSnapshotCopier::LocalSnapshotCopier()
        : _tid(INVALID_BTHREAD), _cancelled(false), _filter_before_copy_remote(false), _fs(NULL), _throttle(NULL), _writer(NULL), _storage(NULL), _reader(NULL), _cur_session(NULL)
    {
    }
    ...
    void LocalSnapshotCopier::copy()
    {
        do
        {
            ...
            // 对远端的所有文件执行copy_file
            // 这一步和从远端复制meta文件类似，调用RemoteFileCopier::start_to_copy_to_file，然后向远端发rpc请求，
            // leader收到请求之后用request里面的reader把文件读出来放到response_attachment里面，rpc返回后follower将得到的数据放到本地文件里面。
                                                ...
                                                SnapshotReader *last_snapshot)
    {
        ...
        for (size_t i = 0; i < remote_files.size(); ++i)
        {
            ...
            {
                std::string source_path = last_snapshot->get_path() + '/' + filename;
    ...
    void LocalSnapshotCopier::copy_file(const std::string &filename)
    {
        ...
        scoped_refptr<RemoteFileCopier::Session> session = _copier.start_to_copy_to_file(filename, file_path, NULL);
        if (session == NULL)
        {
