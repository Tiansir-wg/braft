...
namespace braft
{
    ...
    static bvar::Adder<int64_t> g_read_entry_from_storage("raft_read_entry_from_storage_count");
    static bvar::PerSecond<bvar::Adder<int64_t>> g_read_entry_from_storage_second("raft_read_entry_from_storage_second", &g_read_entry_from_storage);

    static bvar::Adder<int64_t> g_read_term_from_storage("raft_read_term_from_storage_count");
    static bvar::PerSecond<bvar::Adder<int64_t>> g_read_term_from_storage_second("raft_read_term_from_storage_second", &g_read_term_from_storage);
    ...
    LogManager::LogManager()
        : _log_storage(NULL), _config_manager(NULL), _stopped(false), _has_error(false), _next_wait_id(0), _first_log_index(0), _last_log_index(0)
    {
    ...
    void LogManager::clear_memory_logs(const LogId &id)
    {
        ...
        do
        {
            ...
            {
                BAIDU_SCOPED_LOCK(_mutex);
                while (!_logs_in_memory.empty() && nentries < ARRAY_SIZE(entries_to_clear))
                {
    ...
    void LogManager::unsafe_truncate_suffix(const int64_t last_index_kept)
    {
        ...
        TruncateSuffixClosure *tsc = new TruncateSuffixClosure(last_index_kept, last_term_kept);
        ...
        std::vector<LogEntry *> *entries, StableClosure *done)
    {
        ...
        else
        {
            ...
            {
                done->status().set_error(EINVAL, "There's gap between first_index=%" PRId64 " and last_log_index=%" PRId64,
            ...
            else
            {
                ...
                // ones.
                // 判断这一批entry里面的entry是否有term冲突的
                // conflicting_index记录第一个冲突的位置
                size_t conflicting_index = 0;
                for (; conflicting_index < entries->size(); ++conflicting_index)
                {
                    if (unsafe_get_term((*entries)[conflicting_index]->id.index) != (*entries)[conflicting_index]->id.term)
                    {
                        break;
                    }
                }
    ...
    public:
        AppendBatcher(LogManager::StableClosure *storage[], size_t cap, LogId *last_id,
                      LogManager *lm)
            : _storage(storage), _cap(cap), _size(0), _buffer_size(0), _last_id(last_id), _lm(lm)
        {
