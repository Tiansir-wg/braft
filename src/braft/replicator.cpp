...
namespace braft
{
    ...
    ReplicatorOptions::ReplicatorOptions()
        : dynamic_heartbeat_timeout_ms(NULL), log_manager(NULL), ballot_box(NULL), node(NULL), term(0), snapshot_storage(NULL)
    {
    }

    Replicator::Replicator()
        : _next_index(0), _flying_append_entries_size(0), _consecutive_error_times(0), _has_succeeded(false), _timeout_now_index(0), _heartbeat_counter(0), _append_entries_counter(0), _install_snapshot_counter(0), _readonly_index(0), _wait_id(0), _is_waiter_canceled(false), _reader(NULL), _catchup_closure(NULL)
    {
    ...
    {
        if (options.log_manager == NULL || options.ballot_box == NULL || options.node == NULL)
        {
                                      ...
                                      int64_t rpc_send_time)
    {
        ...
        if (entries_size > 0)
        {
            ...
            {
                g_normalized_send_entries_latency << cntl->latency_us() * 1024 / cntl->request_attachment().size();
            }
        }
    ...
    ReplicatorGroupOptions::ReplicatorGroupOptions()
        : heartbeat_timeout_ms(-1), election_timeout_ms(-1), log_manager(NULL), ballot_box(NULL), node(NULL), snapshot_storage(NULL)
    {
    }
