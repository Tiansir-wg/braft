...
namespace braft
{
    ...
    RemoteFileCopier::Session::Session()
        : _channel(NULL), _file(NULL), _retry_times(0), _finished(false), _buf(NULL), _timer(), _throttle(NULL), _throttle_token_acquire_time_us(1)
    {
        _done.owner = this;
    }
    ...
    void RemoteFileCopier::Session::on_rpc_returned()
    {
        ...
        if (_response.has_read_size() && (_response.read_size() != 0) && FLAGS_raft_allow_read_partly_when_install_snapshot)
        {
