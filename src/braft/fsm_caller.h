...
namespace braft
{
    ...
    public:
        ...
        ////////////
        // 获取乱序apply的最大index
        int64_t max_applied_index() const
        {
            return _max_applied_index.load(butil::memory_order_relaxed);
        }

        // 判断index位置的entry是否存在，存在返回1，不存在返回0，越界返回-1
        int get_bit(int64_t index) const
        {
            // index必须在_last_applied_index和_max_applied_index之间
            if (index <= _last_applied_index || index > _max_applied_index)
            {
                return -1;
            }
            else
            {
                // _bits数组中的第几个数
                int num_index = index / 32 - 1;
                // 该数的第几个比特
                int bit_index = index % 32;
                u_int32_t num = _bits[num_index];
                if ((num >> (31 - bit_index) & 1) == 1)
                {
                    return 1;
                }
                else
        ...
        int set_bit(int64_t index)
        {
            ...
            else
            {
                int num_index = index / 32 - 1;
                int bit_index = index % 32;
                int num = _bits[num_index] ^ (1 << (31 - bit_index));
                _bits[num_index] = num;
            }
        }

        // 拷贝_bits
        int copy_bits(u_int32_t *dst, int len)
        {
            if (len < 16)
            {
                return -1;
    ...
    private:
        ////////////
        // 顺序apply和乱序apply之间这部分entry是否apply
        // !!! 需要修改
        butil::atomic<u_int32_t>
            _bits[16];

        butil::atomic<int64_t> _max_applied_index;
