...
namespace braft
{
    ...
    SnapshotExecutor::SnapshotExecutor()
        : _last_snapshot_term(0), _last_snapshot_index(0), _term(0), _saving_snapshot(false), _loading_snapshot(false), _stopped(false), _snapshot_storage(NULL), _cur_copier(NULL), _fsm_caller(NULL), _node(NULL), _log_manager(NULL), _downloading_snapshot(NULL), _running_jobs(0), _snapshot_throttle(NULL)
    {
    }
    ...
    int SnapshotExecutor::register_downloading_snapshot(DownloadingSnapshot *ds)
    {
        ...
        // newer
        if (m->request->meta().last_included_index() == ds->request->meta().last_included_index())
        {
            // m is a retry
            has_saved = true;
        ...
        else if (m->request->meta().last_included_index() > ds->request->meta().last_included_index())
        {
            // |ds| is older
