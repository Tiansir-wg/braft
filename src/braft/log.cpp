...
namespace braft
{
                             ...
                             size_t size_hint) const
    {
        ...
        RawUnpacker(p).unpack64((uint64_t &)term).unpack32(meta_field).unpack32(data_len).unpack32(data_checksum).unpack32(header_checksum);
        EntryHeader tmp;
        tmp.term = term;
    ...
    int Segment::_get_meta(int64_t index, LogMeta *meta) const
    {
        ...
        if (index > _last_index.load(butil::memory_order_relaxed) || index < _first_index)
        {
            // out of range
    ...
    // 初始化操作主要检查文件信息，将segment的索引信息加载到内存，为读写操作做准备。
    ...
    int SegmentLogStorage::append_entries(const std::vector<LogEntry *> &entries, IOMetric *metric)
    {
        ...
        // 检查日志连续性
        if (_last_log_index.load(butil::memory_order_relaxed) + 1 != entries.front()->id.index)
        {
    ...
    int SegmentLogStorage::truncate_prefix(const int64_t first_index_kept)
    {
        ...
        // 将first_index_kept保存到Log_meta文件中，这样保证了即使后续的文件删除操作失败时，也可以知道整个日志的起始first_log_index是多少
        if (save_meta(first_index_kept) != 0)
        { // NOTE
            PLOG(ERROR) << "Fail to save meta, path: " << _path;
            return -1;
        }
        //  将first_index_kept之前的segment文件全部删除
        std::vector<scoped_refptr<Segment>> popped;
        ...
        for (it = _segments.begin(); it != _segments.end();)
        {
                     ...
                     segment->first_index() != last_log_index + 1)
            {
                ...
                return -1;
                     ...
                     _first_log_index.load(butil::memory_order_acquire) < segment->first_index())
            {
    ...
    // 加载segment，从segment中获取_last_log_index
    int SegmentLogStorage::load_segments(ConfigurationManager *configuration_manager)
    {
        ...
        // closed segments
        SegmentMap::iterator it;
        // 加载所有的封闭范围的segment, 更新_last_log_index
