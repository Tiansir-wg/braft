...
namespace example
{
    ...
    public:
        Block()
            : _node(NULL), _leader_term(-1), _fd(NULL)
        {
        }
        ~Block()
        {
        ...
        int start()
        {
            ...
            // raft需要三种不同的持久化存储
            // RaftMetaStorage存储和raft算法自身相关的元数据信息, 包括term、woteFor等数据
            // LogStorage用于存放用户提交的WAL日志
            // SnapshotStorage用于存放快照信息
    ...
    private:
        ...
        // @braft::StateMachine
        // 操作日志commit之后通知用户将操作应用到业务状态机中
        // 通过iter, 可以遍历所有未应用到状态机但是已经提交的日志
        void on_apply(braft::Iterator &iter)
        {
            ...
            for (; iter.valid(); iter.next())
            {
                ...
                if (iter.done())
                {
                    ...
                    // 将请求中的数据写入data变量,后面会将data变量写入文件
